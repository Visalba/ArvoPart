...
Clock.schedule(lambda: #Comienza en el beat 4
		p1 >> viola(notes, dur=[1,2], 
				oct=5, amp=0.1), Clock.now()+4)
Clock.schedule(lambda: #Comienza en el beat 7
		p2 >> viola(getTvoiceFromMelody(notes, 4), dur = [1,2], 
				oct=5), Clock.now()+4)
#Violins II
Clock.schedule(lambda: 
		v1 >> viola(notes, dur=[2,4], 
				oct = 4), Clock.now()+7)
Clock.schedule(lambda: 
		v2 >> viola(getTvoiceFromMelody(notes, 4), dur = [2,4], 
				oct = 4), Clock.now()+7)
#Violas
Clock.schedule(lambda: 
		l3 >> viola(notes, dur=[4,8], 
				oct = 4), Clock.now()+13)
#Cellos
Clock.schedule(lambda: 
		c1 >> viola(notes, dur=[8,16], 
				oct = 3), Clock.now()+25)
Clock.schedule(lambda: 
		c2 >> viola(getTvoiceFromMelody(notes, 4), dur = [8,16], 
				oct = 3), Clock.now()+25)
#Contrabasses
Clock.schedule(lambda: 
		b1 >> viola(notes, dur=[16,32], 
				oct = 2), Clock.now()+50)
Clock.schedule(lambda: 
		b2 >> viola(getTvoiceFromMelody(notes, 4), dur = [16,32], amp=0.1, 
				oct = 2), Clock.now()+50)
